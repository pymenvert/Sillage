...
| Compression | zstd | enregistrements |
...
|---|---|---|
| `build-test` | PR, push main | matrice {windows-latest, ubuntu-22.04} × {Debug, Release} : build, tests unitaires, lint C++ et TS, build UI |
...
| | Windows | Ubuntu |
|---|---|---|
| Format | installeur Inno Setup | paquet `.deb` |
| Service | Windows Service (recovery auto) | unité systemd (`Restart=on-failure`, hardening : User dédié, ProtectSystem) |
| Accès série/réseau | driver USB-série constructeur documenté | groupe `dialout`, règles udev fournies |
| Données | `%ProgramData%\Sillage` | `/var/lib/sillage` |
| UI | navigateur → `http://localhost:8080` | idem |
